        ...
        fetch('https://joxcastro.github.io/listaPlantasApi/categoria/planta-interior.json')
            ...
            .then(data => {
                $.each(data,function(i,item){
                    ...
                    $("#productoCiclo").append(`<div class="producto" type="submit" id="enviar" value="Ver producto"><form action="{% url 'planta' %}" id="formPlanta"><div class="producto__imagen"><input type="text" name="categoria" id="categoria" value="${item.categoria}" style="display: none;"> <input type="text" name="id" id="id" value="${item.id}" style="display: none;"><img src="${item.img}" alt="imagen planta"></div><div class="producto__informacion"><p class="producto__nombre">${item.nomPlant}</p><p class="producto__precio">\$${precio}</p><input type="submit" class="btn btn-primary" id="enviar" value="Ver producto"></div></form></div>`);
                    if(item.id == id){
